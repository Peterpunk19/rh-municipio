...
const IncidentsByTypeChart = ({ date, isLoading, title }: Props) => {
  const theme = useTheme();
  const today = new Date().toISOString().slice(0, 10);
  const [data, setData] = useState<IncidentByType[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    setLoading(true);
    fetch(`/api/dashboard/incidents?createdAt=${date}`)
      .then(res => res.json())
  ...
  return (
    <DashboardCard
      title={title ?? 'Incidencias por tipo'}
    >
