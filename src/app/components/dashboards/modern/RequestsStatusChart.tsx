...
const RequestsStatusChart = ({ date, isLoading, title }: Props) => {
  const theme = useTheme();
  const [data, setData] = useState<RequestStatusItem[]>([]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`/api/dashboard/requests/status?createdAt=${date}`)
      .then(res => res.json())
  ...
  return (
    <DashboardCard title={title ?? 'Estatus de Solicitudes'}>
