...
export default async function seedDC_LV_Nocturno_OmisionSalidaInasistencia(numberEmployee: string) {
  const { employee, location, ascription, employeeAttendanceType } = await createEmployeeSeed({
    numberEmployee,
    name: "DC_Nocturno_OmisionSalidaInasistencia",
  ...
  const { scheduleMap } = await generateMonthlyAttendances({
    ...
    month: 12,
    untilDay: 15,
    excludeDays: ["2025-12-03", "2025-12-04"],
    jornadas: {
      lunesViernes: {
        weekdays: [1, 2, 3, 4, 5],
        startDay: 1,
