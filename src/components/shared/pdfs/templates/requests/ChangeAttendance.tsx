...
const ChangeAttendance: React.FC<RequestTemplateProps> = ({ data, type, styles }) => {
  ...
  return (
    <>
      ...
      <View style={combineStyles(styles?.flexColumn, styles?.alignItemsStart, { fontSize: 10, marginVertical: 15 })}>
        <Text>
          Por medio del presente, solicito la autorización para que se considere el registro en lista de asistencia al
          personal con funciones operativas que, por la naturaleza de las funciones encomendadas no podrá cumplir con el
          registro de asistencia en el reloj digital.
        </Text>
      </View>
      <View style={styles?.table}>
        ...
        <View style={styles?.row}>
          ...
          <View style={combineStyles(styles?.cell, { width: "37.5%" })}>
            {data?.employee?.job_schedule_employee &&
              data?.employee?.job_schedule_employee.map((item: any, index: number) => (
                <Text key={index}>
          ...
          <View style={combineStyles(styles?.cell, styles?.lastCell, { width: "37.5%" })}>
            {data?.employee?.job_schedule_employee &&
              data?.employee?.job_schedule_employee.map((item: any, index: number) => (
                <Text key={index}>
        ...
        <View style={styles?.row}>
          <View style={combineStyles(styles?.cell, styles?.lastRow, { width: "25%" })}>
            <Text style={styles?.textUpperCase}>Fecha de periodo</Text>
          </View>
          <View style={combineStyles(styles?.cell, styles?.lastCell, styles?.lastRow, { width: "75%" })}>
            <Text style={styles?.textUpperCase}>
              {data?.requestDetail?.start_date && data?.requestDetail?.end_date
                ? `${formatDateToText(data?.requestDetail?.start_date, true)} al ${formatDateToText(data?.requestDetail?.end_date, true)}`
                : ""}
            </Text>
          </View>
        </View>
      ...
      <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 60 }}>
        ...
        <View style={{ width: "33.33%", alignItems: "center" }}></View>
        <View style={{ width: "33.33%", alignItems: "center" }}>
