...
describe("API: /employee-requests/update", () => {
  ...
  testCases.forEach(({ description, requestData, expectedStatus, expectedResponse }) => {
    ...
    it(`PUT /employee-requests/update ${description}`, async () => {
      if (description.includes("empty id") || description.includes("id not number") || description.includes("empty status")) {
      }
      else if (description.includes("id zero") || description.includes("id negative") || description.includes("id too long")) {
